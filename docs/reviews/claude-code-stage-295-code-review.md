...
claude --effort max --permission-mode plan --no-session-persistence \
  ...
  -p "$(cat docs/reviews/claude-code-stage-295-code-review-prompt.md)"
```
...
Per `docs/REVIEW_PROTOCOL.md`, Stage 295 code review falls back to local opencode (`zhipuai-coding-plan/glm-5.2 --variant max`).
