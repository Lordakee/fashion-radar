# Claude Code Stage 311 Code Review Prompt

You are reviewing `/home/ubuntu/fashion-radar` on branch `main`.

Review the current uncommitted Stage 311 implementation. Keep the review
concise. Inspect only `git diff -- README.md docs/row-one.md
src/fashion_radar/row_one/templates.py tests/test_row_one_docs.py
tests/test_row_one_render.py` and the five changed files. Focus on correctness,
contract safety, test coverage, escaping/link safety, and whether the code
...
1. Inspect the current diff for the five changed files only.
