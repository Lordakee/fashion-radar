# opencode Stage 264 Code Review Prompt

You are the fallback code reviewer for Fashion Radar Stage 264. Review in read-only mode. Do not edit files.

## Context

Claude Code primary code review timed out after 300 seconds and did not produce a committed review body. Perform an independent code review of the current Stage 264 diff.
...
6. Does first-run smoke coverage remain local-first and avoid default repo artifacts?
...
Return one coherent review body with sections:
...
Do not include command logs, tool chatter, duplicated verdicts, or partial drafts. If you find Critical or Important issues, include concrete fixes required before release.
