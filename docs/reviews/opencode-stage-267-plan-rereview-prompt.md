Review the revised Stage 267 plan before implementation.
...
Prior review: docs/reviews/opencode-stage-267-plan-review.md
...
- The strict `expected_first_run_flow_commands(...)` command sequence is updated
  with the `row-one serve --dry-run` tuple between `row-one preview` and
  `row-one local-ops`.
- `docs/row-one.md` preserves the exact phrase `row-one serve --dry-run`.
- The accidental inlined review prompt block was removed from the plan.
