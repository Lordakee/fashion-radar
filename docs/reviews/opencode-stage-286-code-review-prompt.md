...
Claude Code was attempted first with `--effort max`, but local authentication failed with `401 API key is disabled`. Per project review protocol, this is the opencode fallback code review.

## Review Fixes Already Applied After Parallel Read-Only Review

Please specifically verify these fixes:

1. `edition_brief` no longer advertises `#briefing-path` unless the same path blocks are renderable by the homepage after excluding read-first duplicate cards.
2. `row-one status` now rejects missing or drifted top-level `edition_brief` values for `row-one-app/v5`.
3. `scripts/check_first_run_smoke.py` now rejects missing or drifted top-level `edition_brief` values.
4. Docs no longer claim unchanged JSON contract semantics where Stage 286 intentionally bumped the app contract to `row-one-app/v5`.
5. The edition brief dek CSS selector no longer loses styling when the lead story line is present.
