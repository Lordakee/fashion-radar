# Stage 287 Plan Review Prompt (opencode fallback)
...
Claude Code was attempted first with `--effort max`, but local authentication failed with `401 API key is disabled`. Per project review protocol, this is the opencode fallback plan review.
...
- Evaluate whether the plan correctly bumps to `row-one-app/v6` for the new required top-level app field.
- Evaluate deterministic derivation from existing story payload fields: `entity_refs`, `product_refs`, `designer_refs`, `heat_delta`, `evidence_count`, section membership, and safe detail links.
- Verify local-observed / review-required wording boundaries and no market-demand/platform-coverage claims.

## Review Questions

1. Is this the right next product node for the user request to organize fashion information rather than only links?
...
4. Are the schema, status/smoke validation, homepage rendering, docs, and tests scoped correctly?
5. Are there missing RED tests, escape tests, schema drift cases, local-observed boundaries, or release hygiene risks?
...
For each Critical or Important finding, include the file/plan section, why it matters, and the smallest fix.
