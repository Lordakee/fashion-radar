...
def test_rejects_sdist_without_row_one_runtime_schema(tmp_path: Path) -> None:
    build_dir = tmp_path / "dist"
    build_dir.mkdir()
    write_wheel(build_dir)
    write_sdist(
        build_dir,
        files=[path for path in SDIST_FILES if path != "schemas/row-one-runtime.schema.json"],
    )
    ...
    assert result.returncode == 1
    assert (
        "sdist archive missing required file: schemas/row-one-runtime.schema.json" in result.stderr
    )
    assert "Traceback" not in result.stderr
